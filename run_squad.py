...
class SquadExample(object):
    ...
    def __repr__(self):
        s = ""
        s += "qas_id: %s" % (bpe_tokenization.printable_text(self.qas_id))
        s += ", question_text: %s" % (
            bpe_tokenization.printable_text(self.question_text))
                ...
                if is_training:
                    ...
                    if not is_impossible:
                        ...
                        cleaned_answer_text = " ".join(
                            bpe_tokenization.whitespace_tokenize(orig_answer_text))
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if example_index < 20:
                ...
                tf.logging.info("tokens: %s" % " ".join(
                    [bpe_tokenization.printable_text(x) for x in tokens]))
                ...
                if is_training and not example.is_impossible:
                    ...
                    tf.logging.info(
                        "answer: %s" % (bpe_tokenization.printable_text(answer_text)))
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if not FLAGS.version_2_with_negative:
            ...
            if best_non_null_entry:
                ...
                scores_diff_json[example.qas_id] = 0.0
                all_predictions[example.qas_id] = ""
...
def get_final_text(pred_text, orig_text, do_lower_case):
    ...
    tokenizer = bpe_tokenization.BasicTokenizer(do_lower_case=do_lower_case)
